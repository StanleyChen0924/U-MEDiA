...
def main():
    ...
    type_Execute = 0
    # 取得命令列輸入的 TYPE
    # 如果參數以 'H' 或 'h' 開頭，則去掉該字母
    raw_param = sys.argv[1]
    if raw_param.upper().startswith('H'):
        hex_string = raw_param[1:]
        # 將 16 進位字串轉換為 10 進位整數（H200 轉換後會變成整數 512）
        type_param = int(hex_string, 16)
    else:
        hex_string = raw_param
        type_param = int(hex_string, 10)
        #type_param = int(sys.argv[1])
        ...
        with db.cursor() as cursor:
            ...
            if type_param & 0x100:
                ...
                if result is not None:
                    ...
                    print(clean_msg(f"❌ 找不到 CSN 為 [{ursor.rowcount}] 的紀錄"))
                    ##print(f"FAIL")
                    return
